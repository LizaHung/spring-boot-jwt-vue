    ...
    <div class="p-2 card">
      ...
      <b-table
       ...
       >
        ...
        <template #cell(pdf)>  <b-button class="mr-1" variant="light" size="sm"> 檢視</b-button> </template>
        <template #cell(action)="data">
          <template>
            <b-button v-if="tabIndex === 0" class="mr-1" variant="light" size="sm" @click="showMsgBox(data.item.adoPetNo)">審核</b-button>
            <span v-else>--</span>
          </template>
        </template>
      </b-table>
  ...
  methods: {
    ...
    showMsgBox(adoPetNo) {
      this.$bvModal
        ...
        .then((value) => {
          if (!value) return;
          modalAction.updateStatus( adoPetNo,{adoStatus:'ADOPTED'}, () => {
            this.makeToast("審核成功");
